...
local SessionInfoTableModel = DbBase:extend()
...
function SessionInfoTableModel:get_array_dict_asc(src_tb, dst_tb)
    if not dst_tb then
        dst_tb = {}
    end
...
function SessionInfoTableModel:new()
    local db_name = "db_ai_interview"   --数据库名称
    local tb_name = "ts_session_info"  --表名称
...
function SessionInfoTableModel:Query(query_item, condition_tb, start_num, num, order_by)
    local condition = ""
...
function SessionInfoTableModel:Insert(info)
    local tb = {
        session_id = info.session_id,
        status = info.status or 1,
        position = info.position,
        topic_list = info.topic_list,
        create_time = ngx.now() * 1000,
...
function SessionInfoTableModel:Update(update_tb, condition_tb)
...
function SessionInfoTableModel:Delete(condition_tb)
