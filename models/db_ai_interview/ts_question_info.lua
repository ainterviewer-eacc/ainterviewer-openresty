...
local QuestionInfoTableModel = DbBase:extend()
...
function QuestionInfoTableModel:get_array_dict_asc(src_tb, dst_tb)
    if not dst_tb then
        dst_tb = {}
    end
...
function QuestionInfoTableModel:new()
    local db_name = "db_ai_interview"   --数据库名称
    local tb_name = "ts_question_info"  --表名称
...
function QuestionInfoTableModel:Query(query_item, condition_tb, start_num, num, order_by)
    local condition = ""
...
function QuestionInfoTableModel:Insert(info)
    local tb = {
...
function QuestionInfoTableModel:Update(update_tb, condition_tb)
...
function QuestionInfoTableModel:Delete(condition_tb)
