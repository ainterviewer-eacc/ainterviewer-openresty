...
local function gc_lock(cdata)
    ...
    if key_id > 0 then
        ...
        -- local ok, err = redis_util.string_del(redis_conf.cache, DB_LOCK_NAME, key)
        if not ok then
...
function _M.lock(self, key)
    ...
    local exptime = self.exptime or 20
    ...
    -- local ok, err = redis_util.string_setnx_v2(redis_conf.cache, DB_LOCK_NAME, key, true, exptime)
    ...
    while timeout > 0 do
        ...
        if ok then
            cdata.key_id = ref_obj(key)
            --nlog.info("[db_cache] lock2 cdata.key_id="..cdata.key_id.." cdata.dict_id="..cdata.dict_id.." key="..key)
            return elapsed
        end
        if err ~= "exists" then
            return nil, err
...
function _M.unlock(self)
    local dict = self.dict
    local cdata = self.cdata
    --nlog.info("[db_cache] unlock cdata.key_id="..cdata.key_id.." cdata.dict_id="..cdata.dict_id)
    ...
    local key = memo[key_id]
    unref_obj(key_id)
    --nlog.info("[db_cache] unlock memo="..cjson.encode(memo))
     local ok, err = dict:delete(key)
     if not ok then
         return nil, err
     end
    ...
    cdata.key_id = 0
    --nlog.info("[db_cache] unlock2 cdata.key_id="..cdata.key_id.." cdata.dict_id="..cdata.dict_id)
    return 1
end
